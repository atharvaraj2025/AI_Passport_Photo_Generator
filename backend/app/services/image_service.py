...
from app.models.background_remover import BackgroundRemover, BackgroundRemovalError
...
class PassportImageService:
    ...
    def process_file(self, source: Path, original_filename: str, background_mode: str = "original", background_color: str | None = None) -> ProcessedPhoto:
        ...
        if background_mode == "solid":
            try:
                image = self.background_remover.remove(image)
            except BackgroundRemovalError as exc:
                raise ImageProcessingError(str(exc), "background_removal_failed") from exc
        cropped, _cropped_face = self._passport_crop(image, largest)
        if background_mode == "solid":
            cropped = self._apply_solid_background(cropped, background_color or self.settings.solid_background_color)
    ...
    def _apply_solid_background(self, image: Image.Image, color: str) -> Image.Image:
        """Composite an RGBA portrait over a solid RGB passport background."""
        try:
            rgb = ImageColor.getrgb(color)
        except ValueError as exc:
            raise ImageProcessingError(f"Invalid background color: {color}", "invalid_background_color") from exc

        rgba = image.convert("RGBA")
        background = Image.new("RGBA", rgba.size, (*rgb[:3], 255))
